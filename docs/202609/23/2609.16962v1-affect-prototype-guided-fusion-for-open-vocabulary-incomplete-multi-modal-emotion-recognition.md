...
title_zh: 情感原型引导的开放词汇不完整多模态情感识别融合
authors: "Yichi Zhang, Shenyue Wang, Jing Luo, Chunyang Yu, Xinyu Yang"
date: 2026-09-15
pdf: "https://arxiv.org/pdf/2609.16962v1"
tags: ["query:eeg-align"]
score: 6.0
evidence: 面向缺失模态的多模态情感识别与跨模态融合
tldr: 开放词汇多模态情感识别（OV-MER）旨在从多模态情感线索生成开放自然语言标签，但现实场景中模态缺失与不同步问题突出，现有方法难以有效融合。本文提出情感原型条件融合框架APCF，通过构建情感原型库显式建模不同情感的模态贡献，实现任意情感语义引导下的动态约束融合，并利用LLM解码器生成开放标签。在OV-MERD+和MER-FG数据集上，APCF显著优于现有最优基线。该工作为不完整模态下的开放词汇情感识别提供了候选无关的生成式融合方案。
source: arxiv
...
motivation: 现实场景中模态数据常缺失且不同步，现有OV-MER方法难以在缺失模态下有效融合，且当前不完整模态融合方法局限于固定标签识别，无法满足开放词汇情感语义引导的需求。
method: 提出候选无关的生成式APCF框架，构建情感原型库显式建模多模态贡献，基于可用模态特征进行条件检索与特征聚合，并将融合表示输入LLM解码器生成开放词汇情感标签。
result: 在OV-MERD+和MER-FG数据集上，APCF显著优于现有最优基线，验证了其在缺失模态下开放词汇情感识别的有效性。
conclusion: APCF为不完整模态下的开放词汇情感识别提供了候选无关的生成式融合方案，拓展了模态贡献学习在任意情感语义引导下的应用。
---

## 摘要
开放词汇多模态情感识别（OV-MER）旨在根据多模态情感线索生成开放的自然语言情感标签。然而，在现实场景中，由于采集设备的限制和用户隐私约束，完整且同步的模态数据难以获取。现有的OV-MER方法大多针对全模态输入设计，在模态缺失条件下无法进行有效的特征融合。同时，当前面向不完整模态设计的融合方法主要关注固定标签识别场景，无法满足在OV-MER场景中由任意情感语义引导融合情感线索的需求。为解决这些挑战，本文提出了一种面向不完整开放词汇情感识别的情感原型条件融合（APCF）框架。作为一个无候选的生成式框架，APCF将模态贡献学习扩展到由任意情感语义引导的场景。具体而言，我们构建了一个情感原型库，以显式建模与不同情感相对应的多模态贡献特征，从而为不同情感语义视角下的模态融合提供动态约束。基于可用的模态特征进行条件检索和特征聚合。随后，将细化后的融合情感表示输入LLM解码器，以生成开放词汇情感标签。在OV-MERD+和MER-FG数据集上的实验表明，APCF显著优于最先进的基线。
